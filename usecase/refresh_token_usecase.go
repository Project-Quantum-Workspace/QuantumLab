...
func (rtu *refreshTokenUsecase) CreateAccessToken(user *model.User, roles []int, secret string, expiry int) (accessToken string, err error) {
	return tokenutil.CreateAccessToken(user, roles, secret, expiry)
}

func (rtu *refreshTokenUsecase) CreateRefreshToken(user *model.User, roles []int, secret string, expiry int) (refreshToken string, err error) {
